@extends('layouts.app')
@section('title', 'Objectifs Immédiats')
@section('page-title', 'Objectifs Immédiats')

@section('breadcrumbs')
    <li><i class="fas fa-chevron-right mx-2 text-xs"></i></li>
    <li class="text-gray-700 font-medium">Objectifs immédiats</li>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500">{{ $objectifs->total() }} objectif(s) immédiat(s)</p>
        @can('papa.modifier')
        <a href="{{ route('objectifs-immediats.create', request()->only('papa_id', 'action_prioritaire_id')) }}"
           class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
            <i class="fas fa-plus"></i> Nouvel objectif immédiat
        </a>
        @endcan
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="flex items-start justify-between flex-wrap gap-3">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <form action="{{ route('objectifs-immediats.destroy', $oi) }}" method="POST"
                          onsubmit="return confirm('Supprimer cet objectif ? Ses résultats attendus seront aussi supprimés.')">
                        @csrf @method('DELETE')
        ...
        <div class="p-12 text-center">
            ...
            <p class="text-gray-400">Aucun objectif immédiat trouvé.</p>
        </div>
        @endforelse
    </div>
