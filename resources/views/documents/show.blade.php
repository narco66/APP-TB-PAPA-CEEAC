    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <div class="flex items-start space-x-5">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    <span class="px-2 py-0.5 rounded text-xs font-medium {{ $document->statut === 'valide' ? 'bg-green-100 text-green-700' : ($document->statut === 'archive' ? 'bg-gray-100 text-gray-600' : ($document->statut === 'obsolete' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-700')) }}">
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm mt-3">
                    <div>
                        <span class="text-xs text-gray-400">Categorie</span>
                    ...
                    <div>
                        <span class="text-xs text-gray-400">Depose le</span>
                        <p class="font-medium text-gray-700">{{ $document->created_at->format('d/m/Y a H:i') }}</p>
                    </div>
                    <div>
                        <span class="text-xs text-gray-400">Depose par</span>
                    ...
                    @if($document->statut === 'valide' && $document->validePar)
                        <div>
                            <span class="text-xs text-gray-400">Valide par</span>
        ...
        <div class="mt-5 flex flex-wrap gap-2">
            @can('telecharger', $document)
                <a href="{{ route('documents.download', $document) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition">
            ...
            @can('archiver', $document)
                @if(in_array($document->statut, ['valide', 'brouillon']))
                    <form action="{{ route('documents.archiver', $document) }}" method="POST" class="inline" onsubmit="return confirm('Archiver ce document ? Il passera en lecture seule.')">
                        @csrf
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($document->versions as $v)
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                <a href="{{ route('documents.show', $v) }}" class="text-indigo-600 hover:underline">{{ $v->titre }}</a>
                            </td>
                            <td class="px-4 py-2 text-gray-500">{{ $v->created_at->format('d/m/Y') }}</td>
                            <td class="px-4 py-2">
                                <span class="px-2 py-0.5 rounded text-xs {{ $v->statut === 'valide' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                    {{ ucfirst($v->statut) }}
                                </span>
                            </td>
                        </tr>
