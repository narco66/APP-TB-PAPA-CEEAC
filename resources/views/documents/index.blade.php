...
@section('page-title', 'Gestion Électronique des Documents (GED)')
...
<div class="space-y-6">
    ...
    <form method="GET" class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex flex-wrap gap-3 items-end">
        <div>
            ...
            <input type="text" name="q" value="{{ request('q') }}"
                   ...
                   class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 w-52">
        </div>
        <div>
        ...
        <div>
            ...
            <select name="statut" class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500">
                ...
                <option value="obsolete"   {{ request('statut') === 'obsolete' ? 'selected' : '' }}>Obsolète</option>
            </select>
        </div>
        <button type="submit"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="min-w-0">
                        <div class="flex items-center flex-wrap gap-2 mb-1">
                            <a href="{{ route('documents.show', $doc) }}"
                               class="font-medium text-sm text-gray-800 hover:text-indigo-600">{{ $doc->titre }}</a>
                            <!-- Statut -->
                            <span class="px-2 py-0.5 rounded text-xs font-medium
                                {{ $doc->statut === 'valide' ? 'bg-green-100 text-green-700' :
                                   ($doc->statut === 'archive' ? 'bg-gray-100 text-gray-600' :
                                   ($doc->statut === 'obsolete' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-700')) }}">
                                {{ ucfirst($doc->statut) }}
                            </span>
                            <!-- Confidentialité -->
                        ...
                        <p class="text-xs text-gray-400">
                            ...
                            {{ $doc->tailleLisible() }} •
                            v{{ $doc->version }} •
                            Déposé le {{ $doc->created_at->format('d/m/Y') }}
                            @if($doc->deposePar) par {{ $doc->deposePar->nomComplet() }} @endif
                        </p>
                        @if($doc->reference)
                        <p class="text-xs text-gray-400">Réf : {{ $doc->reference }}</p>
                        @endif
                    </div>
                </div>
                <div class="flex items-center space-x-2">
                    <a href="{{ route('documents.show', $doc) }}"
                       class="px-3 py-1.5 text-gray-600 hover:text-gray-800 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                        <i class="fas fa-eye mr-1"></i>Voir
                    </a>
                    @can('telecharger', $doc)
                    <a href="{{ route('documents.download', $doc) }}"
                       class="px-3 py-1.5 text-indigo-600 hover:text-indigo-800 text-sm border border-indigo-200 rounded-lg hover:bg-indigo-50 transition">
        ...
        <div class="p-12 text-center">
            ...
            @can('document.deposer')
            <a href="{{ route('documents.create') }}" class="mt-4 inline-block text-indigo-600 hover:underline text-sm">
