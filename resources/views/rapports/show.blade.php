...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
    <div class="flex flex-wrap gap-6 text-sm">
        ...
        <div>
            ...
            <span class="font-medium ml-1">{{ $rapport->valide_le->format('d/m/Y a H:i') }}</span>
        </div>
        @endif
        @if($rapport->publie_le)
        <div>
            <span class="text-gray-500">Publie le :</span>
            <span class="font-medium ml-1">{{ $rapport->publie_le->format('d/m/Y a H:i') }}</span>
        </div>
        @endif
    </div>
</div>
...
<div class="space-y-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-red-50 border-b border-red-100 px-5 py-3">
            <h2 class="text-sm font-semibold text-red-800">
                <i class="fas fa-exclamation-triangle mr-2"></i> Difficultes rencontrees
            </h2>
        </div>
        <div class="px-5 py-4">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-400">
        ...
        <p>Aucun contenu narratif renseigne pour ce rapport.</p>
    </div>
    @endif
</div>

@if($rapport->papa && auth()->user()->can('voir', $rapport->papa))
<div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex items-center justify-between">
    <div class="text-sm text-gray-600">
        ...
        Exporter la synthese complete du PAPA <strong>{{ $rapport->papa->code }}</strong> :
    </div>
    <div class="flex items-center gap-3">
        @can('rapport.exporter')
        <a href="{{ route('rapports.export-papa-pdf', $rapport->papa) }}"
           class="inline-flex items-center gap-2 bg-red-100 text-red-700 hover:bg-red-200 px-4 py-2 rounded-lg text-sm font-medium">
            <i class="fas fa-file-pdf"></i> Synthese PDF
        </a>
