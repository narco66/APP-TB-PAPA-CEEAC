...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="bg-red-50 rounded-xl p-4 border border-red-100 text-center">
            <p class="text-3xl font-bold text-red-600">
                {{ $alertes->where('niveau', 'critique')->count() }}
            </p>
        ...
        <div class="bg-yellow-50 rounded-xl p-4 border border-yellow-100 text-center">
            <p class="text-3xl font-bold text-yellow-600">
                {{ $alertes->where('niveau', 'attention')->count() }}
            </p>
        ...
        <div class="bg-blue-50 rounded-xl p-4 border border-blue-100 text-center">
            <p class="text-3xl font-bold text-blue-600">
                {{ $alertes->where('statut', 'nouvelle')->count() }}
            </p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        @endphp
        <div class="px-5 py-4 border-b border-gray-50 border-l-4 {{ $borderColor }} hover:bg-gray-50 transition
            {{ $alerte->statut === 'nouvelle' ? 'bg-yellow-50/30' : '' }}">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start space-x-3 flex-1 min-w-0">
                    <span class="text-xl flex-shrink-0 mt-0.5">{{ $alerte->iconeNiveau() }}</span>
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            <a href="{{ route('alertes.show', $alerte) }}"
                               class="font-medium text-sm text-gray-800 hover:text-indigo-600">{{ $alerte->titre }}</a>
                            ...
                            <span class="px-2 py-0.5 rounded text-xs bg-purple-100 text-purple-700">
                                <i class="fas fa-arrow-up mr-0.5"></i>Escaladée
                            </span>
                            @endif
                ...
                <a href="{{ route('alertes.show', $alerte) }}"
                   class="flex-shrink-0 text-indigo-600 hover:text-indigo-800 text-sm">
        ...
        <div class="p-12 text-center">
            ...
            <p class="text-gray-400">Aucune alerte trouvée.</p>
        </div>
        @endforelse
    </div>
