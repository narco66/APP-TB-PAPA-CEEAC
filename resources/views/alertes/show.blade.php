...
<div class="max-w-3xl space-y-6">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border-l-4 border-{{ $alerte->couleurNiveau() }}-400 border border-gray-100">
            ...
            <div class="flex-1">
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    <span class="px-3 py-1 rounded-full text-sm font-medium
                        bg-{{ $alerte->couleurNiveau() }}-100 text-{{ $alerte->couleurNiveau() }}-700">
                        {{ ucfirst($alerte->niveau) }}
                    </span>
                    <span class="px-2 py-0.5 rounded text-xs font-medium
                        {{ $alerte->statut === 'resolue' ? 'bg-green-100 text-green-700' :
                           ($alerte->statut === 'nouvelle' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600') }}">
                ...
                <div class="grid grid-cols-2 gap-4 mt-4 text-sm">
                    @if($alerte->papa)
                    <div>
                        <p class="text-xs text-gray-400">PAPA concerné</p>
                        <a href="{{ route('papas.show', $alerte->papa) }}"
                    ...
                    @endif
                    <div>
                        <p class="text-xs text-gray-400">Générée le</p>
                        <p class="font-medium text-gray-700">{{ $alerte->created_at->format('d/m/Y à H:i') }}</p>
                    </div>
                    @if($alerte->lue_le)
                    <div>
                        <p class="text-xs text-gray-400">Vue le</p>
                        <p class="font-medium text-gray-700">{{ $alerte->lue_le->format('d/m/Y à H:i') }}</p>
                    </div>
                    @endif
                    @if($alerte->escaladee)
                    <div>
                        <p class="text-xs text-gray-400">Escaladée le</p>
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 text-xs">Générée automatiquement</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="mt-4 p-3 bg-gray-50 rounded-lg text-sm">
                    <p class="text-xs text-gray-400 mb-1">Élément concerné</p>
                    ...
                    <a href="{{ route('activites.show', $alerte->alertable) }}"
                       class="text-xs text-indigo-600 hover:underline mt-1 block">
                        Voir l'activité <i class="fas fa-arrow-right ml-0.5"></i>
                    </a>
                    @endif
                </div>
                @endif
        ...
        <div class="mt-5 flex flex-wrap gap-2">
            ...
            <button onclick="document.getElementById('modal-traiter').classList.remove('hidden')"
            ...
            <button onclick="document.getElementById('modal-escalader').classList.remove('hidden')"
    ...
    <div class="bg-green-50 rounded-xl p-5 border border-green-100">
        <h3 class="font-semibold text-green-800 mb-2">
            <i class="fas fa-check-circle mr-1"></i>Résolution
        </h3>
        <p class="text-sm text-green-700">{{ $alerte->resolution }}</p>
        @if($alerte->traitee_le)
        <p class="text-xs text-green-600 mt-2">
            Résolue le {{ $alerte->traitee_le->format('d/m/Y à H:i') }}
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-5 border-b border-gray-100">
            <h3 class="font-semibold text-gray-700">
                Actions correctives ({{ $alerte->actionsCorrectives->count() }})
            </h3>
        </div>
                ...
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">
                            ...
                            Échéance : {{ $ac->echeance ? $ac->echeance->format('d/m/Y') : '—' }}
                        </p>
                    </div>
                    <span class="px-2 py-0.5 rounded text-xs font-medium
                        {{ $ac->statut === 'realisee' ? 'bg-green-100 text-green-700' :
                           ($ac->statut === 'en_cours' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700') }}">
...
<div id="modal-traiter" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h3 class="text-lg font-bold text-gray-800 mb-4">Résoudre l'alerte</h3>
        <form action="{{ route('alertes.traiter', $alerte) }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Résolution <span class="text-red-500">*</span>
                </label>
            ...
            <div class="flex justify-end space-x-3">
                <button type="button" onclick="document.getElementById('modal-traiter').classList.add('hidden')"
...
<div id="modal-escalader" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
        ...
        <form action="{{ route('alertes.escalader', $alerte) }}" method="POST" class="space-y-4">
            ...
            <div class="flex justify-end space-x-3">
                <button type="button" onclick="document.getElementById('modal-escalader').classList.add('hidden')"
                        class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Annuler</button>
                <button type="submit"
