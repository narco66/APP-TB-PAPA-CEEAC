...
<div class="space-y-6" x-data="{ onglet: 'detail' }">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <div class="flex items-start justify-between flex-wrap gap-4">
            <div class="flex-1">
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    ...
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-{{ $activite->couleurStatut() }}-100 text-{{ $activite->couleurStatut() }}-700">
                        {{ ucfirst(str_replace('_', ' ', $activite->statut)) }}
                    </span>
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-{{ $activite->couleurPriorite() }}-100 text-{{ $activite->couleurPriorite() }}-700">
                ...
                <div class="flex flex-wrap gap-4 text-xs text-gray-500 mt-2">
                    ...
                    @if($activite->service)
                        <span><i class="fas fa-layer-group mr-1"></i>{{ $activite->service->libelle }}</span>
                    @endif
                    <span><i class="fas fa-user mr-1"></i>Resp. : {{ $activite->responsable?->nomComplet() }}</span>
                    @if($activite->pointFocal)
                        <span><i class="fas fa-map-pin mr-1"></i>PF : {{ $activite->pointFocal->nomComplet() }}</span>
                    @endif
                </div>
            </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
            ...
            <button onclick="document.getElementById('modal-avancement').classList.remove('hidden')"
                ...
                <i class="fas fa-sync mr-1"></i>Mettre a jour l'avancement
            </button>
            @endcan
            ...
            <a href="{{ route('documents.create') }}?documentable_type={{ urlencode(App\Models\Activite::class) }}&documentable_id={{ $activite->id }}"
               class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 transition">
                <i class="fas fa-paperclip mr-1"></i>Deposer un document
            </a>
            @endcan
        </div>
    </div>
    ...
    <div class="flex space-x-1 bg-white rounded-xl p-1 shadow-sm border border-gray-100">
        @foreach([
            'detail' => 'Detail',
            'taches' => 'Taches (' . $activite->taches->count() . ')',
            'jalons' => 'Jalons',
            'budget' => 'Budget',
            'documents' => 'Documents (' . $activite->documents->count() . ')',
        ] as $key => $label)
    ...
    <div x-show="onglet === 'detail'" class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
        ...
        <div class="text-sm text-gray-600 space-y-1">
            ...
            <p><span class="font-medium">Objectif immediat :</span> {{ $activite->resultatAttendu?->objectifImmediats?->libelle }}</p>
            <p><span class="font-medium">Resultat attendu :</span> {{ $activite->resultatAttendu?->libelle }}</p>
        </div>
    </div>
    ...
    <div x-show="onglet === 'taches'" class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="px-5 py-3 border-b border-gray-50 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400">Assigne a : {{ $tache->assignee?->nomComplet() ?? 'Non assigne' }}</p>
            </div>
        ...
        @empty
        <div class="p-8 text-center text-gray-400 text-sm">Aucune tache definie.</div>
        @endforelse
    </div>
    ...
    <div x-show="onglet === 'documents'" class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        @empty
        <div class="p-8 text-center text-gray-400 text-sm">Aucun document attache.</div>
        @endforelse
    </div>
...
<div id="modal-avancement" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
        <h3 class="text-lg font-bold text-gray-800 mb-4">Mettre a jour l'avancement</h3>
        <form action="{{ route('activites.avancement', $activite) }}" method="POST" class="space-y-4">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Taux de realisation (0-100%)</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Date debut reelle</label>
                <input type="date" name="date_debut_reelle" value="{{ $activite->date_debut_reelle?->format('Y-m-d') }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Date fin reelle</label>
                <input type="date" name="date_fin_reelle" value="{{ $activite->date_fin_reelle?->format('Y-m-d') }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Notes de mise a jour</label>
